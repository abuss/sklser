...
def test_simple_pipeline():
    ...
    try:
        ...
        if score_match and prediction_match:
            print("✅ SUCCESS: Simple Pipeline works perfectly")
            ...
            print("❌ FAILED: Score or prediction mismatch")
            print(
                f"   Score: {original_score:.6f} -> {deserialized_score:.6f} (match: {score_match})"
            )
...
def test_complex_pipeline():
    ...
    try:
        ...
        if score_match and prediction_match:
            print("✅ SUCCESS: Complex Pipeline works perfectly")
            print(f"   Serialization time: {serialize_time:.3f}ms")
            print(f"   Deserialization time: {deserialize_time:.3f}ms")
            print(f"   Score: {original_score:.6f} -> {deserialized_score:.6f}")
            return True
...
def test_feature_union_pipeline():
    ...
    feature_union = FeatureUnion(
        [("scaler", StandardScaler()), ("pca", PCA(n_components=3, random_state=42))]
    )
    ...
    try:
        ...
        if score_match and prediction_match:
            print("✅ SUCCESS: FeatureUnion Pipeline works perfectly")
            print(f"   Serialization time: {serialize_time:.3f}ms")
            print(f"   Deserialization time: {deserialize_time:.3f}ms")
            print(f"   Score: {original_score:.6f} -> {deserialized_score:.6f}")
            return True
